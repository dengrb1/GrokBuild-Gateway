...
static EMBEDDED_GBG: &[u8] = include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/embedded/gbg.exe"));
...
pub struct GatewayStatus {
  ...
  /// Path of the extracted/running gateway binary (built-in cache or override)
  pub gbg_path: Option<String>,
  /// Always true for this desktop build (gateway is embedded)
  pub embedded: bool,
}
...
impl GatewayManager {
  ...
  pub fn runtime_dir() -> PathBuf {
    if let Ok(local) = std::env::var("LOCALAPPDATA") {
      return PathBuf::from(local).join("GrokBuild-Gateway").join("runtime");
    }
  ...
  pub fn ensure_embedded_gbg() -> Result<PathBuf, String> {
    ...
    fs::create_dir_all(&dir).map_err(|e| format!("创建 runtime dir failed: {e}"))?;
    ...
    if need_write {
      // Write to temp then rename to avoid half-written exe if crash mid-write
      ...
      // On Windows, replace existing file may fail if locked — remove first
      if exe.exists() {
        let _ = fs::remove_file(&exe);
      }
      if let Err(e) = fs::rename(&tmp, &exe) {
        // Fallback: copy if rename fails
        fs::copy(&tmp, &exe).map_err(|e2| format!("install embedded gbg failed: {e} / {e2}"))?;
  ...
  pub fn resolve_gbg(exe_dir: &Path) -> Result<PathBuf, String> {
    ...
    // Dev convenience: repo release/ or beside desktop (optional, not required)
    let external = [
      exe_dir.join("gbg.exe"),
      exe_dir.join("resources").join("gbg.exe"),
      PathBuf::from("release/gbg.exe"),
    ];
    // Prefer embedded always for "built-in" product; only use external if env GBG_USE_EXTERNAL=1
  ...
  pub fn start(&self, exe_dir: &Path) -> Result<GatewayStatus, String> {
    if self.probe_health() {
      // Attach path info even if we didn't spawn
    ...
    cmd.arg("serve")
      ...
      .arg("--host")
      .arg("127.0.0.1")
      // Work from user home so relative paths are stable
      .current_dir(gbg.parent().unwrap_or(exe_dir))
      .stdin(Stdio::null())
      .stdout(Stdio::piped())
      .stderr(Stdio::piped());

    // Proxy shield for child: loopback never proxied; clear outbound proxy env
  ...
  pub fn stop(&self) -> Result<GatewayStatus, String> {
    ...
    if let Some(mut child) = guard.child.take() {
      ...
      // External/orphaned instance — best-effort kill by port is OS-specific; leave message
